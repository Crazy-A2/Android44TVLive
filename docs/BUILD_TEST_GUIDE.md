...
### 4.7 IJKPlayer 原生库缺失（UnsatisfiedLinkError）

原 `ijkplayer-master.zip` 只包含源码，不含预编译 .so 文件。`ijkplayer-armv7a/src/main/libs/` 目录为空，导致：

- `libijkffmpeg.so`、`libijksdl.so`、`libijkplayer.so` 三个文件缺失
- x86 模拟器启动时崩溃：`UnsatisfiedLinkError: couldn't find "libijkffmpeg.so"`

**解决**：从 `leavesCZY/ijkplayer-so-extend`（GitHub）获取预编译 .so 文件：

| 架构 | 模块 | so 文件来源 |
|------|------|-----------|
| x86 | `ijkplayer-x86/` | 新增模块，so 放入 `src/main/libs/x86/` |
| armeabi-v7a | `ijkplayer-armv7a/` | 补充 so 到 `src/main/libs/armeabi-v7a/` |

`settings.gradle` 增加 `include ':ijkplayer-x86'`，app 增加 `implementation project(':ijkplayer-x86')`。
...
### 7.5 第二阶段崩溃（UnsatisfiedLinkError）

下一条 bugreport 在 APK 更新后出现，应用显示"加载中"后闪退：

```
FATAL EXCEPTION: main  PID: 7355
java.lang.UnsatisfiedLinkError: ... couldn't find "libijkffmpeg.so"
    at tv.danmaku.ijk.media.player.IjkMediaPlayer.loadLibrariesOnce(IjkMediaPlayer.java:190)
    at com.tvlive.app.player.PlayerManager.play(PlayerManager.kt:39)
```

**根因**：APK 中缺少 x86 架构的 IJKPlayer 原生库（`libijkffmpeg.so`、`libijksdl.so`、`libijkplayer.so`）。模拟器是 x86 架构，无法加载 armv7a 的 so。

**修复**：见 4.7 节——从预编译源补充 x86 和 armv7a 的 .so 文件。
